...
def run_encode_plan(
    ...
    *,
    ...
    prior = (
        find_resumable_session(preparation.directory, output_dir, preset, crf) if resume else None
    )
    ...
    previously_completed = (
        sum(1 for entry in active_session.entries if entry.status == "success")
        if resumed_from_session
        else 0
    )
    previously_skipped = (
        sum(1 for entry in active_session.entries if entry.status == "skipped")
        if resumed_from_session
        else 0
    )
